...
<html>
<head>
    <title>Opthalmologist | Hospital</title>
...
if(isset($_POST["submit"])){
    ...
    $phone=$_POST["phone"];
    $ad=$_POST["address"];
    $con=mysqli_connect("localhost","root","","opthalmologist");
    $temp=0;
    $query = "SELECT * FROM `tomorrow` WHERE TIME='".$time."'";
    $r=mysqli_query($con,$query);
    if((mysqli_fetch_array($r)["NAME"])==null){
        $update = "UPDATE `tomorrow` SET `name`='".$name."',address='".$ad."',age='".$age."',phone='".$phone."' WHERE time = '".$time."'";
